...
const SettingsPage = () => {
  ...
  const autoGeneratedCategoryCount = useMemo(() => {
    return transactionCategories.filter(c => /^cat-[a-f0-9]{6,12}$/i.test(c.name.trim())).length;
  }, [transactionCategories]);
  ...
  const settingsItems = [
    ...
    { icon: HiOutlineTag, label: t("settings.manageCategories") || "Manage Categories", to: "/settings/categories" },
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {/* Quick Access Hub: Family, Tasks & Categories (2-tap accessibility from anywhere) */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 group-hover:scale-105 transition-transform shrink-0">
                <HiOutlineTag className="h-5 w-5" />
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-bold text-sm text-light-text-primary dark:text-text-primary">
                    {t('settings.categoriesHub')}
                  </h3>
                  {autoGeneratedCategoryCount > 0 ? (
                    <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 dark:bg-amber-950/40 text-amber-600 dark:text-amber-300 animate-pulse">
                      {autoGeneratedCategoryCount} action
                    </span>
                  ) : (
                    <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-indigo-100 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-300">
                      {transactionCategories?.length || 0}
                    </span>
                  )}
                </div>
                <p className="text-xs text-light-text-secondary dark:text-text-secondary">
                  {t('settings.categoriesHubDesc')}
                </p>
              </div>
            </div>
            <HiChevronRight className="h-5 w-5 text-light-text-secondary dark:text-text-secondary group-hover:text-primary group-hover:translate-x-0.5 transition-all" />
      ...
      <Card className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-2 border-b border-slate-100 dark:border-zinc-800/80">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="font-bold text-light-text-primary dark:text-text-primary text-sm sm:text-base">
                  Real-Time Notifications
                </h3>
                <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-100 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                  Live Alerts
                </span>
              </div>
              <p className="text-xs text-light-text-secondary dark:text-text-secondary mt-0.5">
                Configure instant push alerts and medicine reminders
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="self-start sm:self-center px-3.5 py-2 rounded-xl text-xs font-semibold bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900/40 active:scale-95 transition-all flex items-center gap-1.5 min-h-[38px] touch-manipulation"
          >
            <HiOutlineSparkles className="w-3.5 h-3.5 text-blue-500 shrink-0" />
      ...
      {/* Chrome Web App / Home Screen Installation Card */}
      <PWAInstallSection />
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-3 pb-3.5 border-b border-slate-200 dark:border-zinc-700/80">
            <div className="flex items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="font-bold text-xs sm:text-sm text-light-text-primary dark:text-text-primary truncate">
                  {t('settings.dataBackupTitle')}
                </h3>
                <p className="text-[11px] sm:text-xs text-light-text-secondary dark:text-text-secondary truncate">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            ...
            <button
              onClick={async () => {
                const isCloudSynced = !!googleFirebaseUser?.uid;
                const confirmMsg = isCloudSynced
                  ? 'Are you sure you want to clear all your records? This will delete your synced data from cloud storage so you can start fresh.'
                  : 'Are you sure you want to clear your current local session records?';
          ...
          <div className="pt-4 border-t border-slate-200 dark:border-zinc-700/60 text-center text-xs text-light-text-secondary dark:text-text-secondary space-y-1">
            ...
            <p className="text-[11px] opacity-75">Sprout v1.2.8</p>
          </div>
        </div>
      </Card>
