...
export const PWAInstallSection: React.FC<PWAInstallSectionProps> = ({ className = '' }) => {
  ...
  return (
    <>
      <div className={`p-3.5 sm:p-5 rounded-2xl bg-gradient-to-br from-violet-600/10 via-purple-500/5 to-emerald-500/10 dark:from-violet-900/30 dark:via-purple-900/15 dark:to-emerald-950/30 border border-violet-200/80 dark:border-violet-800/40 shadow-sm ${className}`}>
          ...
          <div className="flex items-start sm:items-center gap-3 min-w-0">
            <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-gradient-to-tr from-violet-600 to-indigo-600 text-white flex items-center justify-center shrink-0 shadow-md">
              ...
              <div className="absolute -top-1 -right-1 w-3.5 h-3.5 sm:w-4 sm:h-4 bg-emerald-500 rounded-full border-2 border-white dark:border-zinc-900 flex items-center justify-center">
                <BrandMark className="w-2.5 h-2.5 text-white" variant="monochrome" />
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto pt-1 sm:pt-0">
            {isInstalled ? (
              ...
              <Button
                ...
                variant="primary"
                size="sm"
                icon={<HiOutlineArrowDownTray className={`w-4 h-4 shrink-0 ${isInstalling ? 'animate-bounce' : ''}`} />}
                className="w-full sm:w-auto justify-center"
              >
                {isInstallable ? 'Install Web App' : 'Add to Home Screen'}
              </Button>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4 py-1">
          ...
          <div className="flex items-center gap-3 p-3 rounded-xl bg-violet-50 dark:bg-violet-950/30 border border-violet-200 dark:border-violet-800/40">
            <img 
              src="/icon-192.svg" 
              alt="Sprout App Icon" 
              className="w-12 h-12 rounded-xl shadow-sm border border-violet-200 dark:border-zinc-700" 
            />
            <div>
          ...
          {isInIframe && (
            <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/40 text-xs space-y-2">
              ...
              <button
                ...
                className="w-full py-2 px-3 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-semibold text-xs flex items-center justify-center gap-1.5 transition shadow-sm"
              >
          ...
          <div className="pt-2 flex items-center gap-2">
            ...
            <Button
              ...
              variant="primary"
              size="sm"
              icon={<HiOutlineArrowTopRightOnSquare className="w-3.5 h-3.5" />}
            >
              Direct Link
            </Button>
          </div>
