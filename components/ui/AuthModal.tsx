...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<AuthTab>(isGoogleAuthenticated ? 'profile' : 'login');
        ...
        {isGoogleAuthenticated ? (
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/30 dark:to-teal-950/20 border border-emerald-200 dark:border-emerald-800/40">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1 flex items-center gap-2">
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 dark:text-emerald-300 bg-emerald-100/70 dark:bg-emerald-900/40 px-2 py-0.5 rounded-full">
                    <HiOutlineCheckCircle className="w-3.5 h-3.5" />
                    Firebase Live: sprout-live
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl bg-slate-50 dark:bg-zinc-800/60 p-3.5 border border-slate-200 dark:border-zinc-700 text-xs space-y-2">
              ...
              <div className="flex justify-between items-center text-light-text-secondary dark:text-text-secondary">
                <span>✉️ Gmail Family Invites:</span>
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl font-semibold text-sm text-zinc-800 bg-white border border-slate-300 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100 shadow-xs hover:bg-slate-50 dark:hover:bg-zinc-700/80 transition-all cursor-pointer"
            >
            ...
            <div className="relative flex items-center justify-center my-2">
              ...
              <span className="bg-light-surface dark:bg-surface px-3 text-[11px] font-medium text-light-text-secondary dark:text-text-secondary uppercase">
                or with email
              </span>
            </div>
            ...
            <div className="flex p-1 bg-slate-100 dark:bg-zinc-800 rounded-xl">
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all ${
                  activeTab === 'login'
                    ? 'bg-white dark:bg-zinc-700 text-light-text-primary dark:text-text-primary shadow-xs'
              ...
              <form onSubmit={handleEmailSignIn} className="space-y-3">
                ...
                <div>
                  <div className="flex justify-between items-center mb-1">
                    <label className="block text-xs font-semibold text-light-text-secondary dark:text-text-secondary">
                      Password
                    </label>
                    <button
                      type="button"
                  ...
                  <div className="relative">
                    <HiOutlineLockClosed className="absolute left-3 top-3 h-4 w-4 text-slate-400" />
                    <input
                      type="password"
                      required
                      placeholder="••••••••"
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-2.5 bg-primary text-white text-xs font-semibold rounded-xl hover:bg-primary-focus transition-all shadow-sm flex items-center justify-center gap-1.5 disabled:opacity-50"
                >
            ...
            {activeTab === 'signup' && (
              <form onSubmit={handleSignUp} className="space-y-3">
                <div>
                  ...
                  <div className="relative">
                    <HiOutlineUser className="absolute left-3 top-3 h-4 w-4 text-slate-400" />
                    <input
                      type="text"
            ...
            {activeTab === 'forgot' && (
              <form onSubmit={handleForgotPassword} className="space-y-3">
                <p className="text-xs text-light-text-secondary dark:text-text-secondary">
                  Enter your email address and we'll send you a password reset link from Firebase.
                </p>
                <div>
                  <label className="block text-xs font-semibold text-light-text-secondary dark:text-text-secondary mb-1">
                    Email Address
            ...
            <div className="p-3 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800/40 rounded-xl text-[11px] text-emerald-800 dark:text-emerald-300">
              <span className="font-semibold">🔒 Firebase Security:</span> Your health and family data is synced in real-time to your private collection on <code className="font-mono text-[10px] bg-emerald-100 dark:bg-emerald-900/50 px-1 py-0.5 rounded">sprout-live</code>.
            </div>
          </div>
        )}
      </div>
